...
coord_bump <- function(x,ras,radius = 10){
  ...
  x <- st_as_sf(x,coords=c("lon","lat"),crs=rasproj) #convert to sf
  ...
  if (is.na(depth)) {
    ...
    if (sum(!is.na(values(bathy))) > 1) {
      ...
      bathypoints <- data.frame(
        x = raster::coordinates(bathy)[!is.na(values(bathy)), ][1],
        y = raster::coordinates(bathy)[!is.na(values(bathy)), ][2]
      ) %>%
        st_as_sf(coords = c("x", "y"), crs = rasproj)
    }   # <-- this closes the else
  
    
    nearest_point <- bathypoints[suppressMessages(st_nn(x%>%st_transform(rasproj),bathypoints%>%st_transform(rasproj),progress = FALSE))[[1]],]
